...
model.obj=Objective(expr=profit-Tcost_F_W-Tcost_W_S-
                    1000*sum(u[i,s] for i in range(1,11) for s in range(1,6))
                    ,sense=maximize)
...
print('Optimal_profit',value(model.obj)+1000*value(sum(u[i,s]for i in range(1,11) 
                                        for s in range(1,6))))
