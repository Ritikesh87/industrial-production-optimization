# -*- coding: utf-8 -*-
"""
Created on Tue Aug 25 11:34:14 2025
...
'''Objective function'''
model.obj=Objective(expr=profit-Tcost_F_W-Tcost_W_S-sum(250*mh[j]+300*lh[j] 
                                                        for j in range(1,4))
                    ,sense=maximize)
...
for j in range(1,4):
    ...
    model.C_labor.add(expr=lh[j]<=0.2*C.Labor_Hours_Available[j-1])
...
for j in range(1,4):
    ...
    model.C_machine.add(expr=mh[j]<=0.2*C.Machine_Hours_Available[j-1])
